...
export const updateIn: SafeUpdate = (o: any, ...args: any[]) => {
...
export namespace Prism {
  export function of<T, V>(spec: IPrism<T, V>): Prism<T, V> {
    const func = ((o: T) => spec.get(o)) as Prism<T, V>;

    const set = function set(tOrV: T | V, v?: V) {
      // might want to set v as undefined
      if (arguments.length === 1) {
        return (t: T) => spec.set(t, tOrV as V);
      } else {
        return spec.set(tOrV as T, v!);
      }
    };

    const update = (tOrFn: T | Function, f?: Function) => {
      if (f === undefined) {
        return (t: T) => func.update(t, tOrFn as any);
      } else {
        const t = tOrFn as T;
        const v = spec.get(t);
        if (v === undefined) {
